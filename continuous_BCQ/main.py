...
import DDPG
import utils
from BCQ import add_bonus_for_close_agent
...
# Trains BCQ offline
def train_BCQ(state_dim, action_dim, max_action, device, args,train_env):
# def train_BCQ(state_dim, action_dim, max_action, device, args):
	...
	# Initialize policy
	policy = BCQ.BCQ(state_dim, action_dim, max_action, device, train_env, args.discount, args.tau, args.lmbda, args.phi)

	# Load buffer
	replay_buffer = utils.ReplayBuffer(state_dim, action_dim, device)
	replay_buffer.load(f"./buffers/{buffer_name}")

	target_position = train_env.get_target()
	...
	while training_iters < args.max_timesteps: 
		pol_vals = policy.train(replay_buffer, target_position, iterations=int(args.eval_freq), batch_size=args.batch_size)
		...
		# Save model weights
		actor_path = f"./models/BCQ_actor_{setting}_{training_iters}.pth"
		critic_path = f"./models/BCQ_critic_{setting}_{training_iters}.pth"
		torch.save(policy.actor.state_dict(), actor_path)
		torch.save(policy.critic.state_dict(), critic_path)
	
		# Save M model weights
		m_path = f"./models/BCQ_m_{setting}_{training_iters}.pth"
		torch.save(policy.mnetwork.state_dict(), m_path)

		# Save V model weights
		# m_path = f"./models/BCQ_v_{setting}_{training_iters}.pth"
		# torch.save(policy.vnetwork.state_dict(), m_path)

		# Save losses
		np.save(f"./results/Critic_Loss_{setting}", policy.critic_losses)
		np.save(f"./results/M_Loss_{setting}", policy.m_losses)
...
if __name__ == "__main__":
	...
	parser.add_argument("--max_timesteps", default=3e6, type=int)   # Max time steps to run environment or train for (this defines buffer size)
	...
	if args.train_behavioral or args.generate_buffer:
		...
		train_BCQ(state_dim, action_dim, max_action, device, args,env)
